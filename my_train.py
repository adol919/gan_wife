if __name__ == '__main__':
    ...
    # #打印图像
    # real_batch = next(iter(dataloader))#这里的意思应该是迭代一次取出batch_size个图像数据
    # plt.figure(figsize=(8,8))#指定窗口大小
    # plt.axis("off")#关闭坐标轴
    # plt.title("Training Images")
    # plt.imshow(np.transpose(vutils.make_grid(real_batch[0], padding=0, normalize=True),
    #                         # make_grid的作用是吧若干张图拼接成一张图片,normal参数是改为0-1
    #                         (1,2,0)#trnaspose本质作用是交换坐标维度
    #
    #                         )
    #            )
    # plt.show()

    def weights_init(m):  # 输入一个模型，初始化其中的卷积层，卷积转置层，batch标准化层
        classname = m.__class__.__name__
        if classname.find('Conv') != -1:
            nn.init.normal_(m.weight.data, 0.0, 0.02)
        elif classname.find('BatchNorm') != -1:
            nn.init.normal_(m.weight.data, 1.0, 0.02)
            # 指出所有模型权重应从正态分布中随机初始化，mean=0,stdev = 0.02
    ...
    netG.apply(weights_init)  # 递归的应用该函数
